...
//
   ...
   // R1: X=NL
   //     Y: NL, NS, ZE, PS, PL
   fl->rules[0].inp_fuzzy_set[0] = in_nl; fl->rules[0].inp_fuzzy_set[1] = in_nl; fl->rules[0].out_fuzzy_set = out_nm; // 0,0 -> NM
   fl->rules[1].inp_fuzzy_set[0] = in_nl; fl->rules[1].inp_fuzzy_set[1] = in_ns; fl->rules[1].out_fuzzy_set = out_nl; // 0,1 -> NL
   fl->rules[2].inp_fuzzy_set[0] = in_nl; fl->rules[2].inp_fuzzy_set[1] = in_ze; fl->rules[2].out_fuzzy_set = out_nvl; // 0,2 -> NVL
   fl->rules[3].inp_fuzzy_set[0] = in_nl; fl->rules[3].inp_fuzzy_set[1] = in_ps; fl->rules[3].out_fuzzy_set = out_nvl; // 0,3 -> NVL
   fl->rules[4].inp_fuzzy_set[0] = in_nl; fl->rules[4].inp_fuzzy_set[1] = in_pl; fl->rules[4].out_fuzzy_set = out_nl; // 0,4 -> NL
   ...
   // R3: X=ZE
   //     Y: NL, NS, ZE, PS, PL
   fl->rules[10].inp_fuzzy_set[0] = in_ze; fl->rules[10].inp_fuzzy_set[1] = in_nl; fl->rules[10].out_fuzzy_set = out_pl; // 2,0 -> PL
   fl->rules[11].inp_fuzzy_set[0] = in_ze; fl->rules[11].inp_fuzzy_set[1] = in_ns; fl->rules[11].out_fuzzy_set = out_pm; // 2,1 -> PM
   fl->rules[12].inp_fuzzy_set[0] = in_ze; fl->rules[12].inp_fuzzy_set[1] = in_ze; fl->rules[12].out_fuzzy_set = out_ze; // 2,2 -> ZE
   fl->rules[13].inp_fuzzy_set[0] = in_ze; fl->rules[13].inp_fuzzy_set[1] = in_ps; fl->rules[13].out_fuzzy_set = out_nm; // 2,3 -> NM
   fl->rules[14].inp_fuzzy_set[0] = in_ze; fl->rules[14].inp_fuzzy_set[1] = in_pl; fl->rules[14].out_fuzzy_set = out_nl; // 2,4 -> NL
   ...
   // R5: X=PL
   //     Y: NL, NS, ZE, PS, PL
   fl->rules[20].inp_fuzzy_set[0] = in_pl; fl->rules[20].inp_fuzzy_set[1] = in_nl; fl->rules[20].out_fuzzy_set = out_nm; // 4,0 -> NM
   fl->rules[21].inp_fuzzy_set[0] = in_pl; fl->rules[21].inp_fuzzy_set[1] = in_ns; fl->rules[21].out_fuzzy_set = out_pm; // 4,1 -> PM
   fl->rules[22].inp_fuzzy_set[0] = in_pl; fl->rules[22].inp_fuzzy_set[1] = in_ze; fl->rules[22].out_fuzzy_set = out_pvl; // 4,2 -> PVL
   fl->rules[23].inp_fuzzy_set[0] = in_pl; fl->rules[23].inp_fuzzy_set[1] = in_ps; fl->rules[23].out_fuzzy_set = out_pl; // 4,3 -> PL
   fl->rules[24].inp_fuzzy_set[0] = in_pl; fl->rules[24].inp_fuzzy_set[1] = in_pl; fl->rules[24].out_fuzzy_set = out_pm; // 4,4 -> PM
      return;
}
...
void initMembershipFunctions(fuzzy_system_rec *fl) {
   ...
   fl->inp_mem_fns[INPUT_X][in_nl] = init_trapz (-8.0, -6.0, 0.0, 0.0, left_trapezoid);
   // NS (Negative Small) - 正常梯形
   fl->inp_mem_fns[INPUT_X][in_ns] = init_trapz (-6.0, -3.0, -1.0, 0.0, regular_trapezoid);
   // ZE (Zero) - 正常梯形
   fl->inp_mem_fns[INPUT_X][in_ze] = init_trapz (-2.0, -0.5, 0.5, 2.0, regular_trapezoid);
   // PS (Positive Small) - 正常梯形
   fl->inp_mem_fns[INPUT_X][in_ps] = init_trapz (0.0, 1.0, 3.0, 6.0, regular_trapezoid);
   // PL (Positive Large) - 右梯形
   fl->inp_mem_fns[INPUT_X][in_pl] = init_trapz (6.0, 8.0, 0.0, 0.0, right_trapezoid);
   ...
   // NL (Negative Large) - 左梯形
   fl->inp_mem_fns[INPUT_Y][in_nl] = init_trapz (-4.0, -2.0, 0.0, 0.0, left_trapezoid);
   // NS (Negative Small) - 正常梯形
   fl->inp_mem_fns[INPUT_Y][in_ns] = init_trapz (-3.0, -1.0, -0.3, 0.0, regular_trapezoid);
   // ZE (Zero) - 正常梯形
   fl->inp_mem_fns[INPUT_Y][in_ze] = init_trapz (-0.5, -0.1, 0.1, 0.5, regular_trapezoid);
   // PS (Positive Small) - 正常梯形
   fl->inp_mem_fns[INPUT_Y][in_ps] = init_trapz (0.0, 0.3, 1.0, 3.0, regular_trapezoid);
   // PL (Positive Large) - 右梯形
   fl->inp_mem_fns[INPUT_Y][in_pl] = init_trapz (2.0, 4.0, 0.0, 0.0, right_trapezoid);
...
void initFuzzySystem (fuzzy_system_rec *fl) {
   ...
   coefficient_D=15.0;
	...
	// **修改这里：定义规则输出常数 (力的值)**
	fl->output_values [out_nvl]=-15.0; // NVL (Negative Very Large)
	fl->output_values [out_nl] = -12.0; // NL
	fl->output_values [out_nm] = -8.0;  // NM
	fl->output_values [out_ns] = -4.0;  // NS
	fl->output_values [out_ze] = 0.0;   // ZE
	fl->output_values [out_ps] = 4.0;   // PS
	fl->output_values [out_pm] = 8.0;   // PM
	fl->output_values [out_pl] = 12.0;  // PL
	fl->output_values [out_pvl]= 15.0;  // PVL (Positive Very Large)
